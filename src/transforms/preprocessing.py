...
class HTCSigNetPreprocessor:
    ...
    def __init__(self, transforms, canvas_size: Tuple[int, int] = (1100, 1700),
    ...
    def __call__(self, img: np.ndarray) -> np.ndarray:    
        ...
        centered = self.normalize_image(img, self.canvas_size)
        inverted = 255 - centered
        resized = self.resize_image(inverted, self.img_size)

        if self.input_size is not None and self.input_size != self.img_size:
            cropped = self.crop_center(resized, self.input_size)
        else:
            cropped = resized

        torch_cropped = torch.from_numpy(cropped)
        return self.transforms(torch_cropped)
    ...
    def normalize_image(self, img: np.ndarray,
        ...
        if img_rows > max_rows:
            ...
            print("Img", img_rows, img_cols, img.shape)
    ...
    def crop_center(self, img: np.ndarray,
                    size: Tuple[int, int]) -> np.ndarray:
