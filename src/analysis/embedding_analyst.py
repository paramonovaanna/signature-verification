...
class EmbeddingAnalyzer:
    def __init__(
        ...
        Args:
            ...
            dataset: Dataset instance containing signatures
            preprocessor: HTCSigNetPreprocessor instance
            device: Device to run the model on
            batch_size: Batch size for inference
            num_samples_per_person: Number of genuine signatures to sample per person
        """
        self.device = device
    ...
    def get_embeddings(self, images: torch.Tensor) -> torch.Tensor:
        ...
        with torch.no_grad():
            ...
            features = self.model.model.features(images)
            features = self.model.model.avgpool(features)
    ...
    def calculate_distances(self) -> Tuple[np.ndarray, np.ndarray]:
        ...
        for person_id, signatures in tqdm(self.person_signatures.items(), desc="Processing people"):
            ...
            # Preprocess reference signature
            ...
            all_labels.extend([1] * len(distances[0]))  # All are genuine
            ...
            if forged_signatures:
                ...
                all_distances.extend(forged_distances[0])
                all_labels.extend([0] * len(forged_distances[0]))
